...
void stack_test_internal_popping( void )
{
  ...
  */
  ...
  lfds_stack_new( &ss, 1000000 );

  for( loop = 0 ; loop < 1000000 ; loop++ )
    lfds_stack_push( ss, (void *) (lfds_atom_t) loop );

  stps = malloc( sizeof(struct stack_test_popping_state) * cpu_count );
  for( loop = 0 ; loop < cpu_count ; loop++ )
  {
    (stps+loop)->ss = ss;
    lfds_stack_new( &(stps+loop)->ss_thread_local, 1000000 );
  }
  ...
  for( loop = 0 ; loop < cpu_count ; loop++ )
    while( lfds_stack_pop((stps+loop)->ss_thread_local, (void **) &count) )
      (*(found_count+count))++;
  ...
  free( found_count );
  for( loop = 0 ; loop < cpu_count ; loop++ )
    lfds_stack_delete( (stps+loop)->ss_thread_local, NULL, NULL );
  free( stps );
...
thread_return_t CALLING_CONVENTION stack_test_internal_thread_popping( void *stack_test_popping_state )
{
  ...
  lfds_stack_use( stps->ss );

  while( lfds_stack_pop(stps->ss, (void **) &count) )
    lfds_stack_push( stps->ss_thread_local, (void *) count );
...
void stack_test_internal_pushing( void )
{
  ...
  */
  ...
  for( loop = 0 ; loop < cpu_count ; loop++ )
  {
    (stps+loop)->thread_number = (lfds_atom_t) loop;
    (stps+loop)->ss = ss;
  }
  ...
  lfds_stack_query( ss, LFDS_STACK_QUERY_VALIDATE, &vi, (void *) dvs );

  while( dvs[0] == LFDS_VALIDITY_VALID and lfds_stack_pop(ss, (void **) &user_data) )
  {
...
thread_return_t CALLING_CONVENTION stack_test_internal_thread_pushing( void *stack_test_pushing_state )
{
  ...
  // TRD : we write (thread_number | counter), where thread_number is the top 8 bits of the lfds_atom_t
  while( lfds_stack_push(stps->ss, (void **) ((stps->thread_number << (sizeof(lfds_atom_t)*8-8)) | counter++)) );
...
void stack_test_internal_popping_and_pushing( void )
{
  ...
  */
  ...
  lfds_stack_new( &ss, 100000 * cpu_count * 2 );
  ...
  {
    (stpps+loop)->ss = ss;
    lfds_stack_new( &(stpps+loop)->local_ss, 100000 );

    (stpps+loop+cpu_count)->ss = ss;
    lfds_stack_new( &(stpps+loop+cpu_count)->local_ss, 100000 );

    // TRD : fill the pushing thread stacks
    for( subloop = 0 ; subloop < 100000 ; subloop++ )
      lfds_stack_push( (stpps+loop+cpu_count)->local_ss, (void *) (lfds_atom_t) subloop );
  }
  ...
  for( loop = 0 ; loop < cpu_count * 2 ; loop++ )
    lfds_stack_delete( (stpps+loop)->local_ss, NULL, NULL );

  free( stpps );

  vi.min_elements = vi.max_elements = 100000 * cpu_count * 2;

  lfds_stack_query( ss, LFDS_STACK_QUERY_VALIDATE, (void *) &vi, (void *) dvs );
...
thread_return_t CALLING_CONVENTION stack_test_internal_thread_popping_and_pushing_start_popping( void *stack_test_popping_and_pushing_state )
{
  ...
  lfds_stack_use( stpps->ss );
  lfds_stack_use( stpps->local_ss );
  ...
  while( time(NULL) < start_time + 10 )
  {
    ...
    while( count < 100000 )
      if( lfds_stack_pop(stpps->ss, &user_data) )
      {
        lfds_stack_push( stpps->local_ss, user_data );
        count++;
      }

    // TRD : return our local stack to the main stack
    while( lfds_stack_pop(stpps->local_ss, &user_data) )
      lfds_stack_push( stpps->ss, user_data );
  }
...
thread_return_t CALLING_CONVENTION stack_test_internal_thread_rapid_popping_and_pushing( void *stack_state )
{
  ...
  ss = (struct lfds_stack_state *) stack_state;
  ...
  while( time(NULL) < start_time + 10 )
  {
    lfds_stack_push( ss, user_data );
    lfds_stack_pop( ss, &user_data );
  }
