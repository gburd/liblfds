...
struct lfds_freelist_element *lfds_ringbuffer_get_read_element( struct lfds_ringbuffer_state *rs, struct lfds_freelist_element **fe )
{
  ...
  lfds_queue_dequeue( rs->qs, (void **) fe );
...
struct lfds_freelist_element *lfds_ringbuffer_get_write_element( struct lfds_ringbuffer_state *rs, struct lfds_freelist_element **fe, int *overwrite_flag )
{
  ...
  /* TRD : we try to obtain an element from the lfds_freelist
           if we can, we populate it and add it to the lfds_queue

           if we cannot, then the lfds_ringbuffer is full
           so instead we grab the current read element and
           use that instead

           dequeue may fail since the lfds_queue may be emptied
           during our dequeue attempt

           so what we actually do here is a loop, attempting
           the lfds_freelist and if it fails then a dequeue, until
           we obtain an element

           once we have an element, we lfds_queue it
           ...
           so for example the pop could fail because the lfds_freelist
           is empty; but by the time we go to get an element from
           the lfds_queue, the whole lfds_queue has been emptied back into
           the lfds_freelist!

           if overwrite_flag is provided, we set it to 0 if we
           obtained a new element from the lfds_freelist, 1 if we
           stole an element from the lfds_queue
  */
  ...
  do
  {
    ...
    lfds_freelist_pop( rs->fs, fe );

    if( *fe == NULL )
    {
      lfds_ringbuffer_get_read_element( rs, fe );
